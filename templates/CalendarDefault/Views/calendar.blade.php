      ...
      <div class="calendar-list col-12">
        <div class="nav nav-pills mb-3" role="tablist">
          @foreach($calendar_years as $key => $calendar_year)
            <a class="nav-link {{ ($key == 0) ? 'active' : '' }}" data-bs-toggle="pill" href="#year-{{ $calendar_year->slug }}" role="tab" aria-controls="year-{{ $calendar_year->slug }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}}">{{ $calendar_year->name }}</a>
          @endforeach
        </div>

        <div class="tab-content">
          @foreach($calendar_years as $key => $calendar_year)
            <div class="tab-pane fade {{ ($key == 0) ? 'show active' : '' }}" id="year-{{$calendar_year->slug}}" role="tabpanel" aria-labelledby="year-{{ $calendar_year->slug }}-tab">
